...
- Secret `adhoc-way-platform-agent-keys` — the shared API key the instances answer
  with (or `agentKeys.existingSecret`).
...
and `--set agentKeys.secret.llm=...`.

The instances read the agent key from this Secret and do not create anything of
...
| `agentKeys.existingSecret` | `""` | Secret with the shared API key read by the instances |
