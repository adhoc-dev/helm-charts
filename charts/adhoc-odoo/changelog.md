...
- Bases `fuse`: se revierte la `livenessProbe` exec de #110. El container `adhoc-odoo`
  vuelve a `tcpSocket:8069`; en el `-nx`, la `livenessProbe` exec y el `readinessProbe:9080`
  (ambos agregados por #110, no existían antes) se eliminan → queda sin probe, como antes de
  #110. La reproducción mostró que un restart de container no re-monta el gcsfuse (mount
  pod-scoped): el exec no auto-recuperaba y podía crashloopear (`StartError` en el mount
  anidado `checklist`). La detección y recuperación pasan a una alerta de Prometheus + un
  controller que recrea el pod. (El `readinessProbe:9080` del `-nx` es ortogonal al mount y
  se puede re-agregar aparte si se quiere.)
